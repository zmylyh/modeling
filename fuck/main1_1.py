    ...
    for b_ in block:
        for num in range(3):
            if combination.get(str(simulation((b_, p, y + str(num))))) is not None:
                expression_1 += query_data_1(b_, p, production_data, '亩产量/斤') * combination[
                    simulation((b_, p, y + str(num)))]
    return expression_1
...
for b in block:
    for p in plant:
        for s1 in season:
            for s2 in season:
                if combination.get(str(simulation((b, p, s1)))) is not None and combination.get(
                        str(simulation((b, p, s2)))) is not None:
