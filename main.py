...
combination = LpVariable.dict('comb', [(b, p) for b in block for p in plant], lowBound=0, cat='Continuous')
for b in block:
        problem += lpSum([combination[b,p] for p in plant]) <= area_data[area_data['地块名称'] == b]['地块面积/亩'].values[0]

# = sum 植物price*面积*亩产量 - 植物cost*面积
problem += lpSum([production_data.query(f'作物编号=={p} & 地块类型 == {b}') for b in block for p in plant]*combination[b,p]*price_data.query(f'作物编号=={p} & 地块类型 == {b}')['max'] - cost_data.query(f'作物编号=={p} & 地块类型 == {b}')['种植成本/(元/亩)']*combination[b,p] for b in block for p in plant)
problem.solve()
